...
function injectBootstrap(html, sourceSha) {
  ...
  return html.replace(/<head([^>]*)>/i, `<head$1>\n<!-- ZEFIROK LOCAL BUILD 2.0 · source-sha256:${sourceSha} -->\n${bootstrap}`);
}
...
for (const [sourceName, outputName, kind] of targets) {
  ...
  else html = html.replace(/<title>([^<]*)<\/title>/i, (_m, title) => `<title>${title.replace(/\s*·\s*LOCAL.*$/i,'')} · LOCAL 2.0</title>`);
