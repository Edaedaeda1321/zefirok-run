...
const mustNot=(label,source,needle)=>{const ok=!source.includes(needle);console.log(`${ok?'PASS':'FAIL'}  ${label}`);if(!ok)process.exitCode=1;};
...
must('owner art hidden state',owner,'id="${prefix}ArtUrl" type="hidden"');
must('owner asset picker control',owner,'data-achievement-art-choose="${index}"');
must('owner opens shared asset picker',owner,'openProjectAssetPicker(`${prefix}ArtUrl`)');
mustNot('legacy manual URL input removed',owner,'placeholder="/assets/achievements/badges/....webp"');
