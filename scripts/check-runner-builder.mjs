...
must('strict runner config reader',worker,'async function readRunnerBuilderConfigStrict(env,force=false)');
must('strict active season reader',worker,'async function runnerBuilderActiveSeasonIdStrict(env)');
must('strict public runner reader',worker,'async function readRunnerScenePublicConfigStrict(env)');
must('runner fallback is tagged',worker,'source:"fallback"};');
must('dedicated runner public route',worker,'url.pathname === "/api/game/runner-scene" && request.method === "GET"');
must('dedicated runner public handler',worker,'async function getRunnerSceneConfig(env)');
must('startup isolates runner recovery',worker,'fallback.runnerScene = await readRunnerScenePublicConfigStrict(env);');
must('maintenance allows runner config recovery',worker,'path === "/api/game/runner-scene"');
must('test project runner config path',worker,'"/api/game/runner-scene","/api/achievements"');
...
must('client dedicated runner config path',index,'const RUNNER_SCENE_CONFIG_PATH = &quot;/api/game/runner-scene&quot;;');
must('client dedicated runner loader',index,'async function loadRemoteRunnerScene(force = false)');
must('client rejects invalid runner payload',index,'!Array.isArray(raw.obstacles) || raw.obstacles.length === 0');
must('client retries fallback runner scene',index,'if (!runnerSceneLoaded) void loadRemoteRunnerScene(force);');
must('startup failure reloads runner scene',index,'loadRemoteRunnerScene(force)');
