...
'''
anchor = "async function ownerPanelShop(env, ctx) {"
if worker.count(anchor) != 1:
    raise SystemExit(f"worker featured insert anchor count={worker.count(anchor)}")
worker = worker.replace(anchor, shop_featured_code + anchor, 1)
...
worker = replace_once(worker, route_old, route_new, "owner route")
...
preferred_pattern = re.compile(r'(?m)^(\s*)const preferred = \[[^\n]*"case-alex"[^\n]*\]\.find\([^\n]+\);')
match = preferred_pattern.search(index)
if not match:
    raise SystemExit("featured case preferred anchor not found")
indent = match.group(1)
preferred_replacement = indent + 'const configuredFeaturedCaseId = String(remoteShopFeatured?.case?.itemId ?? "case-alex");\n' + indent + 'const preferred = configuredFeaturedCaseId && shopProducts[configuredFeaturedCaseId]?.enabled !== false && shopProducts[configuredFeaturedCaseId]?.caseType ? configuredFeaturedCaseId : "";'
index, n = preferred_pattern.subn(preferred_replacement, index, count=1)
if n != 1:
    raise SystemExit(f"featured case preferred replacements={n}")

featured_inner_anchor = '        rewardsV2FeaturedEl.innerHTML = `<div class="rewards-v2-featured-art"><img src="${escapePurchaseText(image)}" alt="${escapePurchaseText(product.title)}"></div><div class="rewards-v2-featured-copy"><span class="rewards-v2-featured-badge">Рекомендуем · ${escapePurchaseText(meta.label)}</span><h3>${escapePurchaseText(product.title)}</h3>'
featured_inner_repl = '        const featuredBadgeText = String(remoteShopFeatured?.case?.badgeText || "").trim();\n        const featuredBadgeMarkup = featuredBadgeText ? `<span class="rewards-v2-featured-badge">${escapePurchaseText(featuredBadgeText)}</span>` : "";\n        rewardsV2FeaturedEl.innerHTML = `<div class="rewards-v2-featured-art"><img src="${escapePurchaseText(image)}" alt="${escapePurchaseText(product.title)}"></div><div class="rewards-v2-featured-copy">${featuredBadgeMarkup}<h3>${escapePurchaseText(product.title)}</h3>'
index = replace_once(index, featured_inner_anchor, featured_inner_repl, "featured case badge")
...
skin_return_anchor = '        return `<div class="skins-v2-featured-art" style="--featured-bg:url(\'${skin.background || ""}\')"><img src="${skin.image}" alt="Скин ${skin.title}" loading="eager"></div><div class="skins-v2-featured-copy"><span class="skins-v2-featured-kicker">Рекомендуем · ${escapePurchaseText(rarity.label)}</span><h3>'
skin_return_repl = '        const featuredKickerText = String(remoteShopFeatured?.skin?.badgeText || "").trim();\n        const featuredKicker = featuredKickerText ? `<span class="skins-v2-featured-kicker">${escapePurchaseText(featuredKickerText)}</span>` : "";\n        return `<div class="skins-v2-featured-art" style="--featured-bg:url(\'${skin.background || ""}\')"><img src="${skin.image}" alt="Скин ${skin.title}" loading="eager"></div><div class="skins-v2-featured-copy">${featuredKicker}<h3>'
index = replace_once(index, skin_return_anchor, skin_return_repl, "featured skin badge")
