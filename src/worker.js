...
const GAME_UPDATE_TITLE = "Геймплейные скины Зефи";
...
const GAME_UPDATE_NOTES = Object.freeze([
  "Активный скин Зефи теперь меняется прямо в геймплее.",
  "Для каждого образа подключены отдельные изображения stand, run и jump.",
  "После нажатия «Надеть» новый образ применяется уже в следующем забеге без перезагрузки.",
  "Игровые изображения заранее предзагружаются, поэтому при прыжке нет мигания.",
  "При ошибке загрузки автоматически используется стандартная Зефи.",
  "Физика, коллайдер, высота прыжка и игровая позиция не менялись."
]);
